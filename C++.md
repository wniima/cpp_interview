...
## 14.11 定义一个只在栈（堆）上的对象

（1）只在堆上：只能用 new 创建

将析构函数设置为私有。编译器在为类对象分配空间时，会先检查类析构函数的访问性，若析构函数不可访问，则不能在栈上创建对象。

（2）只在栈上：不能用 new 创建

将 new 和 delete 重载为私有。使用 new 创建对象时，首先会调用 malloc 分配空间，然后再调用构造函数生成对象。将 new 设置为私有，就无法使用了。



## 14.12 [代码编译过程](https://zhuanlan.zhihu.com/p/45402323)

总的来说，代码的编译过程分为以下几个过程：预处理 -》编译 -》汇编 -》链接

（1）预处理：拷贝 #include 包含的文件代码，进行 #define 宏定义的替换，处理条件编译指令。经过预处理输出的 .i 文件中，只有常量以及问关键字。

```
-E

// 只激活预处理，不生成文件，要查看的话可以重定向到一个输出文件中
g++ -E main.cpp

g++ -E main.cpp -o main.i
```

（2）编译：通过语法和词法分析，确定所有指令是否符合规则，之后翻译成**汇编代码。**

```
-S

// 只激活预处理和编译，把文件生成汇编代码，生成 .s 文件
g++ -S main.cpp

g++ -S main.i
```

（3）汇编：把汇编语言**生成目标文件**。目标文件中存放的也就是与源程序等效的目标的**机器码**。

```
-c

// 只激活预处理、编译、汇编等过程，生成 .o 文件
g++ -c main.cpp

g++ -c main.s
```

（4）链接：将有关的目标文件链接起来，将 .o 文件转化成可执行文件

```
g++ main.cpp

// .o 的作用是指定目标名称，省缺的情况下生成 a.out
```
